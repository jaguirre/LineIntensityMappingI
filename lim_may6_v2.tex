...
Here we have implicitly assumed that the fluctuations in line emission trace the matter power spectrum with some average bias, $\bar{b_i}(z)$. For our target redshift range, $\bar{b}_i$ is reasonably well-constrained to be between 2 and 3.\citep{Cooray10, Jullo12}. The assumption of a single bias at each redshift does not allow for a variation of the source bias with the host halo mass (and thus luminosity), but XXX
...
where $\sigma_N^2$ is the instrument sensitivity (noise equivalent intensity, or NEI, in units of Jy sr$^{-1}$ s$^{1/2}$, $V_{vox}$ is the volume of a voxel, and $t_{obs}^{vox}$ is the time spent observing on a single voxel. The variance of a measured $k$, $\sigma^2(k)$, is then written as
...
\begin{equation}
\begin{split}
P_N& = \left(\sigma_N^2 A_{pix} \Delta r_{los}^{vox}\right) / \left({\frac{t_{survey}}{n_{beams}/N_{instr}^{spatial}}}\right) \\
& = \left(\sigma_N^2 A_{pix}\Delta r_{los}^{vox}\right) /  \left(\frac{t_{survey} N_{instr}^{spatial}}{A_{survey}/A_{pix}}\right)\\
& = \sigma_N^2 \frac{\Delta r_{los}^{vox} A_{survey}}{t_{survey} N_{instr}^{spatial}}
\end{split}
\label{eq:pnoise}
\end{equation}
...
\hline \hline
$A_{survey}$ (deg$^2$) &\multicolumn{8}{c}{1.0}\\
$D_{ap}$ (m) &\multicolumn{8}{c}{2.5} \\
$R = \lambda_{obs}/\Delta\lambda$ & \multicolumn{8}{c}{450} \\
N of Spectral Channels & \multicolumn{8}{c}{64} \\
N of Spectrometers (instantaneous spatial pixels) & \multicolumn{8}{c}{25} \\
$t_{obs}^{survey}$ (hr)&\multicolumn{8}{c}{200}\\
\hline
$z_{cen}$ for [CII] & \multicolumn{2}{c}{0.63} & \multicolumn{2}{c}{0.88} & \multicolumn{2}{c}{1.16} & \multicolumn{2}{c}{1.48} \\
$V_{voxel}$ (Mpc$^3$ h$^{-3}$ & \multicolumn{2}{c}{0.36} & \multicolumn{2}{c}{0.81} & \multicolumn{2}{c}{1.59} & \multicolumn{2}{c}{2.87} \\
$\bar{S}_{\mathrm{[CII]}}$ (Jy sr$^{-1}$) & \multicolumn{2}{c}{4.56 $\times 10^3$} & \multicolumn{2}{c}{6.33 $\times$ 10$^3$} & \multicolumn{2}{c}{4.05 $\times 10^3$} & \multicolumn{2}{c}{2.55 $\times 10^3$} \\
\hline
\hline
Experimental Platform & \multicolumn{4}{c}{\emph{Atmospheric Balloon}} & \multicolumn{4}{c}{\emph{Cryogenic Satellite}}\\
$\sigma_N$ (10$^7$ Jy sr$^{-1}$ sec$^{1/2}$) & 3.4 & 2.1 & 1.5 & 1.0 & 0.030 & 0.034 & 0.039 & 0.043 \\
Line Sensitivity, $S_\gamma$ (10$^{-18}$ W m$^{-2}$ sec$^{1/2}$) & 15.8 & 11.3 & 9.20 & 7.10 & 0.139 & 0.185 & 0.240 & 0.306 \\
$f_{err}$ & 230 & 90.4 & 88 & 81 & 2.1 & 1.5 & 2.3 & 3.5 \\
 \hline
 \end{tabular}
 \end{center}
 \end{table*}
...
\caption{Signal-to-noise on the clustering term of the [CII] power spectrum $P_{\textrm{[CII], [CII]}}^{clust}$ and number of modes as a function of $k$ at $z=0.88$. The black, blue, and red lines correspond to survey areas of 1.0, 5.3, and 10.0 deg$^2$, respectively. Telescopes with apertures yielding 0.1, 1, and 10 times the fiducial pixel volume, $V_{vox}$, are shown as the dotted, solid, and dashed lines, respectively.}
...
\begin{equation}
\frac{L_{i,min}}{4\pi D_L^2} \Leftrightarrow 5 \times S_{\gamma} t_{vox}^{-1/2}
\end{equation}
...
\hline \hline
$t_{obs}^{vox} f_{err}^2$ ($\times 10^6$) & B11 & $\alpha = -1.0$ & $\alpha$ = -1.5 & $\alpha$ = -2.0 \\
\hline
Atmospheric Balloon & 1.5378700 & 1.5667517 & 9.0373812 & 2.8255556 \\
Cryogenic Satellite & 0.028645231 & 0.029183196 & 0.016833533 &  0.0052630385 \\
\hline
 \end{tabular}
 \end{center}
 \end{table}

The top row Figure~\ref{fig:ngal_frac} breaks down the total emission in terms of the number of detectable galaxies. As is clear from comparison of panels in the top and bottom rows, a large sample of galaxies (of order 1,000 or greater) does not necessarily ensure an unbiased measure of the mean [CII] intensity. If, however, one extracts the aggregate, unresolved emission from [CII] via the intensity mapped power spectrum, one is essentially measuring $\frac{\rho_{[CII], obs]}}{\rho_{[CII]}} = 1$ as soon as SNR on the linear clustering term of the power spectrum is sufficiently high, which was depicted in Figure~\ref{fig:scii_z}. 
...
\mathrm{SNR}_{IM} & =  \frac{\sqrt{N_{modes}}}{1 + P_N/\left(\bar{S}_{i}^2b_i^2P_{\delta,\delta}\right)} \\
 & = \frac{\sqrt{N_{modes}}}{1 + (f_{err}^2 V_{vox}) / (b_i^2P_{\delta,\delta})} \nonumber
\end{align}
